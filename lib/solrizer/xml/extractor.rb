    ...
    doc.each_pair do |name, value|
      if value.kind_of?(Array) 
        if value.first.kind_of?(Hash)
          ...
          solr_doc.merge!({mapper.solr_name(name, :text, :searchable).to_sym => "#{value.first["content"]}"})
        elsif value.length > 1
          solr_doc.merge!({mapper.solr_name(name, :text, :searchable).to_sym => value})
        else
          solr_doc.merge!({mapper.solr_name(name, :text, :searchable).to_sym => "#{value.first}"})
        end
      else
        solr_doc.merge!({mapper.solr_name(name, :text, :searchable).to_sym => "#{value}"})
      end
